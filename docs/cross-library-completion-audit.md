...
| HarfBuzz/HarfRust | Faster than the faster reference on every representative shaping workload, with independent binaries, pinned CPU, symmetric order, and repeatable margin | `shaping-performance-matrix` | Open: all five maintained core rows lead in three consecutive latest strict runs. The narrow rows are Amiri words at `1.004x--1.008x` and Devanagari at `1.010x--1.013x`; the added long-English suite leads at `1.182x`/`1.103x`, and `react-dom.txt` led by more than `1.50x`. Noto Nastaliq broader coverage exposed a current full-corpus semantic mismatch and remains open |
...
`1.103x`. An attempted Noto Nastaliq addition was not hidden behind performance
numbers: ordinary timed Cangjie output produced 83,663 glyphs versus 83,486 for
both references, so that wider Urdu/Persian semantic differential remains open.
...
the latest shaping run still trails HarfBuzz on Amiri words, while Devanagari
remains a narrow single-font/corpus result. The
