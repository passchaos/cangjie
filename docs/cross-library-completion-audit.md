...
| Parley | Equivalent paragraph layout results—not only counts—and faster default/styled/reflow paths for Latin, Arabic, CJK, bidi, vertical, fallback, and inline-object workloads | `parley-matrix`, paragraph/reflow tests, `docs/text-pipeline.md` | Open: the maintained 26-row matrix covers three scripts, three construction styles, retained reflow, matched in-flow/ordinary out-of-flow objects, and a mixed Roboto→Noto Sans fallback paragraph. All 12 object rows prove identical id/source/line/position/size/baseline geometry at fractional coordinates, and logical-line normalization proves the fallback rows plus 9/24 original text-geometry rows equivalent. The latest fixed-core A/B/B/A run leads every enforced performance row, though Arabic object reflow remains near noise; retained fallback is coverage-only and materially slower, while vertical, custom out-of-flow, and remaining structural differences are open |
...
- `zig build parley-matrix -Doptimize=ReleaseFast -- --iterations 1000 --samples
  7 --cpu 30 --fail-on-slower`: 26/26 count/stability rows passed and all 25
  enforced performance rows passed,
  ...
  The mixed Roboto→Noto Sans Latin/Arabic fallback rows also have identical
  normalized geometry. Construction leads, while retained fallback is
  explicitly coverage-only because Cangjie currently revalidates mutable
  cascade geometry and measured about `0.42x` against Parley's already-resolved
  font assignment; closing that lifecycle/performance gap remains open.
