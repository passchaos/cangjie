...
- `zig build parley-matrix -Doptimize=ReleaseFast -- --iterations 1000 --samples
  ...
  narrow open regressions: Arabic in-flow and out-of-flow retained reflow. A
  direct retained-run marker lookup reduced them to `0.992x` and `0.990x`;
  every other row led. After removing Parley's
