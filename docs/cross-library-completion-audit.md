...
| Parley | Equivalent paragraph layout results—not only counts—and faster default/styled/reflow paths for Latin, Arabic, CJK, bidi, vertical, fallback, and inline-object workloads | `parley-matrix`, paragraph/reflow tests, `docs/text-pipeline.md` | Open: the 18-row matrix covers three scripts, three construction styles, retained reflow, and matched in-flow inline objects; logical-line normalization proves 9/18 geometry rows equivalent; retained paragraph, pure-RTL construction, and strict styled line-building fast paths raise the corrected timing matrix to 16/18 performance rows ahead, while Japanese styled construction remains slower; vertical, fallback, and out-of-flow modes are not comparable |
...
- `zig build parley-matrix -Doptimize=ReleaseFast -- --iterations 1000 --samples
  ...
  Cangjie leads 16/18 rows. Default Latin, Arabic, and Japanese retained reflow
  lead by `1.398x`, `1.044x`, and `1.458x`; their inline-object counterparts
  lead by `1.181x`, `1.192x`, and `1.187x`. Arabic default, spacing,
  alternating, and inline-object construction lead by `1.167x`, `1.143x`,
  `1.011x`, and `1.188x`. Japanese styled
...
16/18 rows, including all six retained-reflow rows, but the remaining two rows
