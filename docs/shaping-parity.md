...
- NotoSansDevanagari parses and focused words `के`, `कि`, `की`, `का`,
  ...
  `अंग्रेज़ी`, and `सिद्धांत` pass `compare-harfrust`. The broader
  `hi-words.txt` corpus now reaches line 1028 and is blocked at `पुनः`, where
  visarga advance differs.
...
- Expand the new Indic shaper slice beyond the current Devanagari `nukt`,
  ...
  stages; `hi-words.txt` is still blocked at visarga advance handling.
