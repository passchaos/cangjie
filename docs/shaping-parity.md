...
- NotoSansDevanagari parses and focused words `के`, `कि`, `की`, `का`,
  ...
  `अंग्रेज़ी`, `सिद्धांत`, and `पुनः` pass `compare-harfrust`. The broader
  `hi-words.txt` corpus now reaches line 2462 and is blocked at `ज़्यादा`, a
  nukta + half-form conjunct case.
...
- Expand the new Indic shaper slice beyond the current Devanagari `nukt`,
  ...
  stages; `hi-words.txt` is still blocked at `ज़्यादा`.
