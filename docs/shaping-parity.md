...
- Amiri `fa-thelittleprince.txt` passes default `compare-harfrust` for 771
  lines.
- NotoNastaliqUrdu `"سلام"` passes `compare-harfrust`; `fa-words.txt` now
  reaches line 3 and is blocked at `به`, where the spacer glyph remains `sp0`
  instead of HarfRust's `sp2`.
...
- Expand the benchmark matrix beyond Amiri, Roboto, SourceSerifVariable, and
  the active Devanagari gate; NotoNastaliqUrdu still needs the `به` spacer
  context mismatch fixed before its `fa-words.txt` corpus can become a retained
  gate.
