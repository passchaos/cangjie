...
fn collectPairAdjustmentLookupAcceleratedImpl(
    ...
    var first_index: usize = 0;
    while (first_index + 1 < glyphs.len) {
        if (!adjacent_pairs and runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[first_index])) {
            first_index += 1;
            continue;
        }
        const second_index = if (adjacent_pairs)
            ...
            nextUnignoredGlyph(glyphs, first_index + 1, lookup_flag, options) orelse {
                first_index += 1;
                continue;
            };
...
fn advanceAfterPairPosition(glyphs: []const GlyphId, first_index: usize, lookup_flag: u16, options: LookupOptions, has_value_2: bool) usize {
    if (!has_value_2) return first_index + 1;

    // HarfBuzz advances beyond the second input only when ValueFormat2 is
    // present. Ignored marks between the pair stay transparent to matching, so
    // derive the consumed glyph from the same skip iterator as PairPos rather
    // than assuming physical adjacency. The caller's next pass then starts
    // after that glyph, preventing overlapping (0,1) and (1,2) applications.
    const second_index = nextUnignoredGlyph(glyphs, first_index + 1, lookup_flag, options) orelse
...
fn collectPairAdjustment(table: Table, subtable_offset: usize, glyphs: []const GlyphId, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!void {
    ...
    while (i + 1 < glyphs.len) {
        ...
        i = advanceAfterPairPosition(glyphs, i, lookup_flag, options, matched and parsed.value_format_2 != 0);
    }
}
...
fn collectPairAdjustmentAtParsed(table: Table, parsed: PairPositionSubtable, glyphs: []const GlyphId, first_index: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    ...
    if (runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[first_index])) return false;
    ...
    switch (parsed.pos_format) {
        1 => {
            ...
            const coverage = try table_core.coverage.index(table, parsed.coverage_offset, glyphs[first_index]) orelse return false;
            ...
            const value_1 = try positioning.value_record.read(table, pair_record + 2, parsed.value_format_1, pair_set_offset);
            const value_2 = try positioning.value_record.read(table, pair_record + 2 + parsed.value_size_1, parsed.value_format_2, pair_set_offset);
        ...
        2 => {
            ...
            if (try table_core.coverage.index(table, parsed.coverage_offset, glyphs[first_index]) == null) return false;
            const class_1 = try table_core.class_def.value(table, parsed.class_def_1, glyphs[first_index]);
            const class_2 = try table_core.class_def.value(table, parsed.class_def_2, glyphs[second_index]);
            ...
            const value_1 = try positioning.value_record.read(table, record_offset, parsed.value_format_1, parsed.subtable_offset);
            const value_2 = try positioning.value_record.read(table, record_offset + parsed.value_size_1, parsed.value_format_2, parsed.subtable_offset);
            try markUnsafePairApplication(
                allocator,
                &options,
                glyphs.len,
...
fn buildChainingCoverageSubtable(table: Table, subtable_offset: usize, allocator: std.mem.Allocator) (GposError || std.mem.Allocator.Error)!?ChainingCoverageSubtable {
    ...
    if (subtable.input_count > 1) {
        ...
        subtable.second_input_digest = try table_core.coverage.digest(table, second_coverage_offset);
    }
    return subtable;
}
...
fn collectMarkToBaseAdjustmentAtParsed(table: Table, subtable: MarkToBaseSubtable, glyphs: []const GlyphId, mark_position: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions, attached_marks: []const bool, search_state: ?*MarkBaseSearchState) (GposError || std.mem.Allocator.Error)!bool {
    ...
    const mark_index = if (subtable.mark_coverage) |coverage|
        ...
        try table_core.coverage.index(table, subtable.mark_coverage_offset, glyph) orelse return false;
    ...
    const base_index = if (subtable.base_coverage) |coverage|
        ...
        try table_core.coverage.index(table, subtable.base_coverage_offset, base_glyph) orelse return false;
...
fn markBaseSearchSkipsGlyphParsed(table: Table, subtable: MarkToBaseSubtable, glyphs: []const GlyphId, index: usize, attached_marks: []const bool, lookup_flag: u16, options: LookupOptions) GposError!bool {
    ...
    if (runtime.matching.matchSkipsGlyph(lookup_flag, options, glyphs, index)) return true;
    ...
    const base_covered = if (subtable.base_coverage) |coverage|
        ...
        try table_core.coverage.index(table, subtable.base_coverage_offset, glyphs[index]) != null;
...
fn previousUnignoredCoveredGlyph(table: Table, coverage_offset: usize, glyphs: []const GlyphId, mark_index: usize, lookup_flag: u16, options: LookupOptions) GposError!?usize {
    ...
    while (i > 0) {
        ...
        if (runtime.matching.matchSkipsGlyph(lookup_flag, options, glyphs, i)) continue;
        return if (try table_core.coverage.index(table, coverage_offset, glyphs[i]) != null) i else null;
    }
    return null;
}
...
fn collectChainingContextAdjustment(table: Table, subtable_offset: usize, glyphs: []const GlyphId, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!void {
    switch (try positioning.lookup.contextual.parseChaining(table, subtable_offset)) {
        .glyph => |subtable| try collectChainingGlyphPositioning(table, subtable, glyphs, adjustments, allocator, lookup_flag, options),
        .class => |subtable| try collectChainingClassPositioning(table, subtable, glyphs, adjustments, allocator, lookup_flag, options),
        .coverage => |parsed| {
            if (parsed.records.input_count == 0) return;
            try collectChainingCoveragePositioning(
                table,
                chainingCoverageSubtable(parsed),
                glyphs,
                adjustments,
                allocator,
                lookup_flag,
...
fn collectChainingContextAdjustmentAt(table: Table, subtable_offset: usize, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    if (pos >= glyphs.len) return false;
    switch (try positioning.lookup.contextual.parseChaining(table, subtable_offset)) {
        .glyph => |subtable| return try collectChainingGlyphPositioningAt(table, subtable, glyphs, pos, adjustments, allocator, lookup_flag, options),
        .class => |subtable| return try collectChainingClassPositioningAt(table, subtable, glyphs, pos, adjustments, allocator, lookup_flag, options),
        .coverage => |parsed| {
            if (parsed.records.input_count == 0) return false;
            return (try collectChainingCoveragePositioningAt(table, chainingCoverageSubtable(parsed), glyphs, pos, adjustments, allocator, lookup_flag, options)).matched;
        },
    }
}

fn parseChainingCoveragePositioningSubtable(table: Table, subtable_offset: usize) GposError!?ChainingCoverageSubtable {
    const parsed = try positioning.lookup.contextual.parseChainingCoverage(
        table,
        subtable_offset,
    ) orelse return null;
    return chainingCoverageSubtable(parsed);
}

fn chainingCoverageSubtable(
    parsed: positioning.lookup.contextual.ChainingCoverage,
) ChainingCoverageSubtable {
    return .{
...
fn collectChainingGlyphPositioning(table: Table, subtable: positioning.lookup.contextual.ChainingGlyph, glyphs: []const GlyphId, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!void {
    var pos: usize = 0;
    while (pos < glyphs.len) {
        var next_pos = pos + 1;
        defer pos = next_pos;
        ...
        if (coverage >= subtable.sets.count) continue;
        const set_offset = try subtable.sets.resolve(table, coverage) orelse continue;
        const result = try collectChainingGlyphRuleSet(table, set_offset, glyphs, pos, adjustments, allocator, lookup_flag, options);
        if (result.matched) next_pos = @max(next_pos, result.next_pos);
    }
}

fn collectChainingGlyphPositioningAt(table: Table, subtable: positioning.lookup.contextual.ChainingGlyph, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    if (pos >= glyphs.len) return false;
    if (runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[pos])) return false;
    const coverage = try table_core.coverage.index(table, subtable.coverage_offset, glyphs[pos]) orelse return false;
    if (coverage >= subtable.sets.count) return false;
    const set_offset = try subtable.sets.resolve(table, coverage) orelse return false;
    return (try collectChainingGlyphRuleSet(table, set_offset, glyphs, pos, adjustments, allocator, lookup_flag, options)).matched;
}

fn collectChainingGlyphRuleSet(table: Table, set_offset: usize, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!PositionContextResult {
    const set = try positioning.lookup.contextual.parseRuleSet(table, set_offset);
    for (0..set.rule_count) |rule_i| {
        const rule = try positioning.lookup.contextual.parseChainingRule(
            table,
            try set.ruleOffset(table, rule_i),
        );
        const backtrack_count = rule.backtrack_count;
        ...
        for (0..backtrack_count) |i| {
            const expected = try readU16(table, rule.backtrack_values_pos + i * 2);
        ...
        const input_count = rule.input_count;
        ...
        for (1..input_count) |i| {
            const expected = try readU16(table, rule.input_values_pos + (i - 1) * 2);
        ...
        const lookahead_count = rule.lookahead_count;
        ...
        for (0..lookahead_count) |i| {
            const expected = try readU16(table, rule.lookahead_values_pos + i * 2);
        ...
        try collectPositionRecordsMapped(table, rule.records.records_pos, rule.records.count, input_indices_buf[0..input_count], glyphs, adjustments, allocator, options);
        return .{ .matched = true, .next_pos = input_indices_buf[input_count - 1] + 1 };
    }
    return .{};
}

fn collectChainingClassPositioning(table: Table, subtable: positioning.lookup.contextual.ChainingClass, glyphs: []const GlyphId, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!void {
    var pos: usize = 0;
    while (pos < glyphs.len) {
        var next_pos = pos + 1;
        defer pos = next_pos;
        ...
        if (input_class >= subtable.sets.count) continue;
        const set_offset = try subtable.sets.resolve(table, input_class) orelse continue;
        const result = try collectChainingClassRuleSet(table, set_offset, subtable.backtrack_class_def, subtable.input_class_def, subtable.lookahead_class_def, glyphs, pos, adjustments, allocator, lookup_flag, options);
        if (result.matched) next_pos = @max(next_pos, result.next_pos);
    }
}

fn collectChainingClassPositioningAt(table: Table, subtable: positioning.lookup.contextual.ChainingClass, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    if (pos >= glyphs.len) return false;
    if (runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[pos])) return false;
    if (try table_core.coverage.index(table, subtable.coverage_offset, glyphs[pos]) == null) return false;
    const input_class = try table_core.class_def.value(table, subtable.input_class_def, glyphs[pos]);
    if (input_class >= subtable.sets.count) return false;
    const set_offset = try subtable.sets.resolve(table, input_class) orelse return false;
    return (try collectChainingClassRuleSet(table, set_offset, subtable.backtrack_class_def, subtable.input_class_def, subtable.lookahead_class_def, glyphs, pos, adjustments, allocator, lookup_flag, options)).matched;
}
...
const ChainingClassMatchContext = struct {
    ...
    fn classValueForRole(self: *ChainingClassMatchContext, role: class_context.ClassRole, glyph: GlyphId) GposError!u16 {
        ...
        return try table_core.class_def.value(self.table, class_def, glyph);
    }

    fn skipsGlyph(self: *ChainingClassMatchContext, glyphs: []const GlyphId, glyph_index: usize) bool {
        return runtime.matching.matchSkipsGlyph(self.lookup_flag, self.options, glyphs, glyph_index);
    }
};
...
fn collectChainingClassRuleSet(table: Table, set_offset: usize, backtrack_class_def: usize, input_class_def: usize, lookahead_class_def: usize, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!PositionContextResult {
    ...
    for (0..set.rule_count) |rule_i| {
        ...
        var matched = true;
        for (0..backtrack_count) |i| {
            const expected_class = try readU16(table, rule.backtrack_values_pos + i * 2);
        ...
        for (1..input_count) |i| {
            const expected_class = try readU16(table, rule.input_values_pos + (i - 1) * 2);
        ...
        for (0..lookahead_count) |i| {
            const expected_class = try readU16(table, rule.lookahead_values_pos + i * 2);
        ...
        try collectPositionRecordsMapped(table, rule.records.records_pos, rule.records.count, input_indices, glyphs, adjustments, allocator, options);
        return .{ .matched = true, .next_pos = input_indices[input_count - 1] + 1 };
    }
    return .{};
}

fn collectChainingCoveragePositioning(table: Table, parsed: ChainingCoverageSubtable, glyphs: []const GlyphId, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!void {
    var pos: usize = 0;
    while (pos < glyphs.len) {
...
fn collectSimpleChainingCoveragePositioningAt(table: Table, subtable: ChainingCoverageSubtable, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!PositionContextResult {
    if (subtable.input_coverages.len != 0) {
        ...
        if (!try table_core.coverage.contains(table, input_coverage_offset, glyphs[pos], .membership)) return .{};
    }
    return try collectSimpleAcceleratedChainingCoveragePositioningAt(table, subtable, glyphs, pos, adjustments, allocator, lookup_flag, options);
}
...
fn collectSimpleAcceleratedChainingCoveragePositioningAt(table: Table, subtable: ChainingCoverageSubtable, glyphs: []const GlyphId, pos: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!PositionContextResult {
    ...
    if (subtable.lookahead_coverages.len != 0) {
        ...
        if (!try table_core.coverage.contains(table, lookahead_coverage_offset, glyphs[lookahead_index], .membership)) return .{};
    }
...
fn gposCoverageSequenceMatches(table: Table, base_offset: usize, glyphs: []const GlyphId, pos: usize, offsets_pos: usize, count: usize, backtrack: bool) GposError!bool {
    ...
    for (0..count) |i| {
        ...
        if (!try table_core.coverage.contains(table, coverage_offset, glyph, .membership)) return false;
    }
    return true;
}
...
fn gposLookaheadCoverageMatches(table: Table, base_offset: usize, glyphs: []const GlyphId, start: usize, offsets_pos: usize, count: usize) GposError!bool {
    ...
    for (0..count) |i| {
        const coverage_offset = try checkedRequiredCoverageOffset(table, base_offset, try readU16(table, offsets_pos + i * 2));
        if (!try table_core.coverage.contains(table, coverage_offset, glyphs[start + i], .membership)) return false;
    }
    return true;
}
...
fn collectMarkToLigatureAdjustmentAt(table: Table, subtable_offset: usize, glyphs: []const GlyphId, mark_position: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    ...
    const mark_index = try table_core.coverage.index(table, mark_coverage_offset, glyph) orelse return false;
    ...
    const ligature_index = try table_core.coverage.index(table, ligature_coverage_offset, glyphs[ligature_position]) orelse return false;
...
fn previousCoveredLigatureGlyph(table: Table, mark_coverage_offset: usize, glyphs: []const GlyphId, mark_position: usize, lookup_flag: u16, options: LookupOptions) GposError!?usize {
    ...
    while (i > 0) {
        i -= 1;
        if (runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[i])) continue;
        if (runtime.matching.markAttachmentSearchSkipsGlyph(options, i)) continue;
...
fn ligatureComponentIndexForMark(table: Table, mark_coverage_offset: usize, glyphs: []const GlyphId, ligature_position: usize, mark_position: usize, component_count: usize, lookup_flag: u16, options: LookupOptions) GposError!usize {
    ...
    while (pos < mark_position) : (pos += 1) {
        if (runtime.matching.lookupIgnoresGlyph(lookup_flag, options, glyphs[pos])) continue;
        if (runtime.matching.markAttachmentSearchSkipsGlyph(options, pos)) continue;
        ...
        if (try table_core.coverage.index(table, mark_coverage_offset, glyphs[pos]) != null) {
...
fn markGlyphForAttachmentSearch(table: Table, mark_coverage_offset: usize, glyph: GlyphId, options: LookupOptions) GposError!bool {
    ...
    return try table_core.coverage.index(table, mark_coverage_offset, glyph) != null;
}
...
fn markGlyphForAttachmentSearchParsed(table: Table, subtable: MarkToBaseSubtable, glyph: GlyphId, options: LookupOptions) GposError!bool {
    ...
    return if (subtable.mark_coverage) |coverage|
        ...
        try table_core.coverage.index(table, subtable.mark_coverage_offset, glyph) != null;
}
...
fn collectMarkToMarkAdjustmentAt(table: Table, subtable_offset: usize, glyphs: []const GlyphId, mark_1_position: usize, adjustments: *std.ArrayList(Adjustment), allocator: std.mem.Allocator, lookup_flag: u16, options: LookupOptions) (GposError || std.mem.Allocator.Error)!bool {
    ...
    const mark_1_index = try table_core.coverage.index(table, mark_1_coverage_offset, glyph) orelse return false;
    ...
    const mark_2_index = try table_core.coverage.index(table, mark_2_coverage_offset, glyphs[mark_2_position]) orelse return false;
