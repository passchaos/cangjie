...
pub const LookupOptions = struct {
    ...
    /// Shared HarfBuzz-compatible resource guards for one top-level GSUB run.
    ///
    /// Contextual lookups can recursively invoke cardinality-changing
    /// substitutions. The guards are optional on low-level detached lookup
    /// tests, but every public top-level application installs both pointers and
    /// all copies of LookupOptions retain them through nested calls.
...
fn buildChainingCoverageLookupAccelerator(table: Table, lookup_offset: usize, subtable_count: u16, extension_wrapped: bool, single_subst: SingleSubstAccelerator, extension_lookup_type_value: ?u16, allocator: std.mem.Allocator) (GsubError || std.mem.Allocator.Error)!LookupAccelerator {
    ...
    for (0..subtable_count) |subtable_i| {
        ...
        if (parsed_subtable.input_count > 1) {
            ...
            chaining_subtables[subtable_i].second_input_digest = try table_core.coverage.digest(table, second_coverage_offset);
        ...
        if (parsed_subtable.input_count > 2) {
            ...
            chaining_subtables[subtable_i].third_input_digest = try table_core.coverage.digest(table, third_coverage_offset);
        }
        if (parsed_subtable.backtrack_count != 0) {
            ...
            chaining_subtables[subtable_i].first_backtrack_digest = try table_core.coverage.digest(table, first_backtrack_coverage_offset);
        }
        if (parsed_subtable.input_count == 1 and parsed_subtable.lookahead_count != 0) {
            ...
            chaining_subtables[subtable_i].first_lookahead_digest = try table_core.coverage.digest(table, first_lookahead_coverage_offset);
        }
        const subtable_digest = try table_core.coverage.digest(table, coverage_offset);
...
fn appendClassFirstIndex(
    ...
    for (0..coverage_count) |coverage_i| {
        const glyph = (try table_core.coverage.glyphAt(table, coverage_offset, coverage_i)) orelse return error.BadGsub;
        const class_set = try table_core.class_def.value(table, input_class_def, glyph);
...
fn appendGlyphFirstIndex(
    ...
    for (0..coverage_count) |coverage_i| {
        ...
        try entries.append(allocator, .{
            .glyph = (try table_core.coverage.glyphAt(table, coverage_offset, coverage_i)) orelse return error.BadGsub,
            .group_index = @intCast(group_index),
        });
    }
    return try appendPreparedClassFirstIndex(entries.items, classes, allocator);
...
fn buildLigatureSubstAccelerator(table: Table, subtable_offset: usize, allocator: std.mem.Allocator) (GsubError || std.mem.Allocator.Error)!LigatureSubstAccelerator {
    ...
    var all_definitions_require_second = true;

    var set_i: usize = 0;
    while (set_i < lig_set_count) : (set_i += 1) {
        const glyph = (try table_core.coverage.glyphAt(table, coverage_offset, set_i)) orelse return error.BadGsub;
        ...
        for (0..ligature_count) |ligature_i| {
            ...
            all_definitions_require_second = all_definitions_require_second and component_count > 1;
            for (1..component_count) |component_index| {
                const component = try readU16(table, ligature_offset + 4 + (component_index - 1) * 2);
                try components.append(allocator, component);
                if (component_index == 1) try second_components.append(allocator, component);
            }
    ...
    const prefilter_second = shouldPrefilterLigatureSecond(competing_definition_count);
    var required_second_start: u32 = 0;
    var required_second_len: u16 = 0;
    if (shouldBuildRequiredLigatureSecondIndex(
        competing_definition_count,
        all_definitions_require_second,
    )) {
        std.sort.heap(GlyphId, second_components.items, {}, glyphIdLessThan);
        deduplicateSortedGlyphs(&second_components);
        if (components.items.len <= std.math.maxInt(u32) and
            second_components.items.len <= std.math.maxInt(u16) and
            second_components.items.len <= std.math.maxInt(u32) - components.items.len)
        {
    ...
    return .{
        ...
        // A compact range denotes a third dispatch mode that still uses the
        // second-prefiltered matcher after its exact run proof. Keep the
        // ordinary prefiltered flag false so unrelated lookups continue
        // through their original specialized worker.
...
fn shouldBuildRequiredLigatureSecondIndex(
    competing_definition_count: usize,
    all_definitions_require_second: bool,
) bool {
    return all_definitions_require_second and
        competing_definition_count >= min_competing_ligature_definitions_for_required_second;
}

fn glyphIdLessThan(_: void, lhs: GlyphId, rhs: GlyphId) bool {
    return lhs < rhs;
}

fn deduplicateSortedGlyphs(glyphs: *std.ArrayList(GlyphId)) void {
...
fn requiredLigatureSecondComponents(accelerator: LigatureSubstAccelerator) []const GlyphId {
    const start: usize = accelerator.required_second_start;
    const len: usize = accelerator.required_second_len;
    if (start > accelerator.components.len or len > accelerator.components.len - start) return &.{};
    return accelerator.components[start .. start + len];
}
...
fn appendChainingSubtablePairs(table: Table, coverage_offset: usize, subtable_index: u16, pairs: *std.ArrayList(ChainingSubtablePair), allocator: std.mem.Allocator) (GsubError || std.mem.Allocator.Error)!void {
    ...
    switch (format) {
        1 => {
            const glyph_count = try readU16(table, coverage_offset + 2);
            if (!table.assume_validated) try table_core.coverage.validateFormat1Order(table, coverage_offset, glyph_count);
...
fn appendChainingPairSubtablePairs(
    ...
    const first_count = try table_core.coverage.glyphCount(table, first_coverage_offset);
    const second_count = try table_core.coverage.glyphCount(table, second_coverage_offset);
    ...
    for (0..first_count) |first_i| {
        const first = (try table_core.coverage.glyphAt(table, first_coverage_offset, first_i)) orelse continue;
        for (0..second_count) |second_i| {
            const second = (try table_core.coverage.glyphAt(table, second_coverage_offset, second_i)) orelse continue;
            pairs.appendAssumeCapacity(.{
                .first = first,
                .second = second,
                .subtable_index = subtable_index,
...
test "GSUB ligature second prefilter cost model counts competing definitions" {
    ...
    try std.testing.expect(shouldBuildRequiredLigatureSecondIndex(
        min_competing_ligature_definitions_for_required_second,
        true,
    ));
    try std.testing.expect(!shouldBuildRequiredLigatureSecondIndex(
        min_competing_ligature_definitions_for_required_second - 1,
        true,
    ));
    try std.testing.expect(!shouldBuildRequiredLigatureSecondIndex(
        min_competing_ligature_definitions_for_required_second,
