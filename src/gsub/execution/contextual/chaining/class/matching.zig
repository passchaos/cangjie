...
pub fn acceleratedGroup(
    ...
    if (group.hash_sorted and group.max_shape_len >= 3) {
        if (lookup_flag == 0 and
            run.run_has_default_ignorables == false and
            !run.match_source_syllable)
        {
            return acceleratedAdjacentHashedGroup(
                view,
                parsed,
                rules,
                glyphs,
...
fn acceleratedAdjacentHashedGroup(
    view: View,
    parsed: Subtable,
    ...
    while (shape_start < rules.len) {
        ...
        var index = low;
        while (index < shape_end and rules[index].hash == hash) : (index += 1) {
            if (matched_index == null or rules[index].order < rules[matched_index.?].order) {
                matched_index = index;
            }
        }
        shape_start = shape_end;
    }
    const rule = rules[matched_index orelse return false];
...
fn adjacentHash(
    ...
    for (0..backtrack_count) |index| {
        hash = class_context.sequenceHashAppend(hash, try table.class_def.valueWithDense(
            view,
            parsed.backtrack_class_def,
            parsed.backtrack_class_values,
            glyphs[position - index - 1],
        ));
    }
    ...
    for (0..lookahead_count) |index| {
        hash = class_context.sequenceHashAppend(hash, try table.class_def.valueWithDense(
            ...
            glyphs[position + input_count + index],
        ));
    }
    return hash;
}
...
fn setAdjacentRule(
    ...
    for (0..backtrack_count) |index| result.backtrack[index] = position - index - 1;
    for (0..input_count) |index| result.input[index] = position + index;
    for (0..lookahead_count) |index| result.lookahead[index] = position + input_count + index;
}
