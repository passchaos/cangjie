//! ContextSubst format-1 glyph rule matching.
...
fn applyRuleSet(
    ...
    for (0..rule_count) |rule_index| {
        ...
        var matched = true;
        for (1..input_count) |component_index| {
            const expected = try view.readU16(
                rule_offset + 4 + (component_index - 1) * 2,
            );
            if (glyphs.items[input_indices[component_index]] != expected) {
                matched = false;
                break;
            }
        }
