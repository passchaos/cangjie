...
test "hyphen resolution prefers U+2010 then portable fallbacks" {
    ...
    const preferred_value = (try resolve(&preferred, 20, &.{})).?;
    ...
    try std.testing.expectEqual(
        @as(u21, 0x002d),
        (try resolve(&ascii, 20, &.{})).?.codepoint,
    );
    ...
    try std.testing.expect((try resolve(&missing, 20, &.{})) == null);
}
