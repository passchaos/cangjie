...
const CharStringScanContext = struct {
    ...
    pub fn blendScalar(self: *CharStringScanContext, vs_index: u16, region_index: usize) Error!f32 {
        const offset = self.vstore_offset orelse return error.BadSfnt;
        return try itemVariationDataDefaultScalar(self.table, offset, vs_index, region_index);
    }
...
fn itemVariationDataDefaultScalar(table: []const u8, vstore_offset: usize, vs_index: u16, item_region_index: usize) Error!f32 {
...
test "CFF2 reads VariationStore scalars for default blend" {
    const bytes = [_]u8{
    ...
    try std.testing.expectEqual(@as(f32, 1), try itemVariationDataDefaultScalar(&bytes, 0, 0, 0));
    try std.testing.expectEqual(@as(f32, 1), try itemVariationDataDefaultScalar(&bytes, 0, 0, 1));
