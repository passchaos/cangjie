    ...
    for (0..row_count) |row_index| {
        ...
        inline for (sample_offsets_4, 0..) |offset, sample_index| {
            ...
            for (row_lines) |line| {
                ...
                const x = @mulAdd(f32, line.slope, py - line.y_min, line.x_at_y_min);
    ...
    while (y <= max_y) : (y += 1) {
        ...
        for (sample_offsets, 0..) |sample_offset, sample_index| {
            ...
            if (row_lines.len == 2) {
                ...
                const first_x = @mulAdd(f32, first.slope, py - first.y_min, first.x_at_y_min);
                const second_x = @mulAdd(f32, second.slope, py - second.y_min, second.x_at_y_min);
            ...
            for (row_lines) |line| {
                if (py < line.y_min or py >= line.y_max) continue;
                const x_intersect = @mulAdd(f32, line.slope, py - line.y_min, line.x_at_y_min);
