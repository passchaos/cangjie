...
pub const SvgDominantBaseline = @import("svg_bridge.zig").SvgDominantBaseline;
pub const SvgResolvedTextSpan = @import("svg_bridge.zig").SvgResolvedTextSpan;
pub const SvgTextAnchor = @import("svg_bridge.zig").SvgTextAnchor;
pub const SvgTextLayout = @import("svg_bridge.zig").SvgTextLayout;
pub const SvgTextMetrics = @import("svg_bridge.zig").SvgTextMetrics;
...
pub const buildSvgTextDrawList = @import("svg_bridge.zig").buildSvgTextDrawList;
...
pub const layoutResolvedSvgText = @import("svg_bridge.zig").layoutResolvedSvgText;
...
test "shapes cascade text right-to-left with visual glyph order" {
    ...
    var layout_buffer = LayoutBuffer.init(allocator);
    defer layout_buffer.deinit();
    const shaped = try TextShaper.shapeUtf8CascadeWithOptions(cascade, &layout_buffer, "AB", 20, .{ .direction = .rtl });

    try std.testing.expectEqual(@as(usize, 2), shaped.glyphs.len);
    try std.testing.expectEqual(@as(u21, 'B'), shaped.glyphs[0].codepoint);
...
test "defaults right-to-left paragraph alignment to the right edge" {
    ...
    try std.testing.expectEqual(@as(usize, 1), paragraph.glyphs[0].cluster);
