...
test "CFF2 top-level metadata is exposed when present" {
    ...
    try std.testing.expectEqual(@as(?usize, 35), info.top_dict.fd_array_offset);
    try std.testing.expectEqual(@as(?usize, 45), info.top_dict.fd_select_offset);
    try std.testing.expectEqual(@as(?usize, 62), info.top_dict.vstore_offset);
    ...
    try std.testing.expectEqualSlices(u8, &.{ 32, 10, 32, 29, 14 }, (try font.cff2CharStringData(0)).?);
