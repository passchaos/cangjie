...
pub fn buildMinimalTtfWithKern(allocator: std.mem.Allocator, kern: []const u8) ![]u8 {
...
pub fn buildFeatTtf(allocator: std.mem.Allocator) ![]u8 {
...
pub fn buildMortContextualTtf(allocator: std.mem.Allocator) ![]u8 {
...
pub fn buildLtagTtf(allocator: std.mem.Allocator) ![]u8 {
    return buildSfnt(allocator, 0x00010000, try ltagTtfTables(allocator));
}
...
fn minimalTtfWithKernTables(allocator: std.mem.Allocator, kern: []const u8) ![]Table {
    const tables = try allocator.alloc(Table, 8);
    var initialized: usize = 0;
    errdefer {
        for (tables[0..initialized]) |table| allocator.free(table.data);
    ...
    tables[4] = .{ .tag = "hmtx", .data = try hmtxTable(allocator) };
    initialized += 1;
    // Kern parser regressions need deliberately malformed pair arrays while
    // keeping the rest of the synthetic SFNT valid.
    tables[5] = .{ .tag = "kern", .data = try allocator.dupe(u8, kern) };
    initialized += 1;
    tables[6] = .{ .tag = "loca", .data = try locaTable(allocator) };
    initialized += 1;
    tables[7] = .{ .tag = "maxp", .data = try maxpTable(allocator) };
...
fn featTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try allocator.alloc(Table, 9);
    errdefer allocator.free(tables);
    tables[0] = .{ .tag = "cmap", .data = try cmapTable(allocator) };
...
fn mortContextualTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try allocator.alloc(Table, 9);
    errdefer allocator.free(tables);
    tables[0] = .{ .tag = "cmap", .data = try cmapFormat12RangeTable(allocator, 65, 66, 1) };
    tables[1] = .{ .tag = "glyf", .data = try emptyGlyfTable(allocator, 3) };
...
fn ltagTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try allocator.alloc(Table, 9);
    errdefer allocator.free(tables);
    tables[0] = .{ .tag = "cmap", .data = try cmapTable(allocator) };
    tables[1] = .{ .tag = "glyf", .data = try glyfTable(allocator) };
    ...
    tables[4] = .{ .tag = "hmtx", .data = try hmtxTable(allocator) };
    tables[5] = .{ .tag = "loca", .data = try locaTable(allocator) };
    tables[6] = .{ .tag = "ltag", .data = try ltagTable(allocator) };
    tables[7] = .{ .tag = "maxp", .data = try maxpTable(allocator) };
    tables[8] = .{ .tag = "kern", .data = try kernTable(allocator) };
    return tables;
}
...
fn vorgTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try allocator.alloc(Table, 9);
    errdefer allocator.free(tables);
    tables[0] = .{ .tag = "VORG", .data = try vorgTable(allocator) };
    tables[1] = .{ .tag = "cmap", .data = try cmapTable(allocator) };
    tables[2] = .{ .tag = "glyf", .data = try glyfTable(allocator) };
    tables[3] = .{ .tag = "head", .data = try headTable(allocator) };
    tables[4] = .{ .tag = "hhea", .data = try hheaTable(allocator) };
...
fn colorV1RadialGradientTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try colorV1GlyphTtfTables(allocator);
    allocator.free(tables[0].data);
...
fn bitmapOnlyCbdtPngTtfTables(allocator: std.mem.Allocator) ![]Table {
    const tables = try allocator.alloc(Table, 8);
    errdefer allocator.free(tables);
    tables[0] = .{ .tag = "CBDT", .data = try cbdtPngTable(allocator) };
    tables[1] = .{ .tag = "CBLC", .data = try cblcPngTable(allocator) };
...
fn mortContextualTable(allocator: std.mem.Allocator) ![]u8 {
    const bytes = try allocator.alloc(u8, 96);
    @memset(bytes, 0);
    writeU32(bytes, 0, 0x0001_0000);
    writeU32(bytes, 4, 1);
    writeU32(bytes, 8, 1);
    writeU32(bytes, 12, 88);
    writeU16(bytes, 16, 0);
    writeU16(bytes, 18, 1);
    writeU16(bytes, 20, 76);
    writeU16(bytes, 22, 0x2001);
    writeU32(bytes, 24, 1);
    ...
    writeU16(bytes, machine + 6, 26);
    writeU16(bytes, machine + 8, 50);
    writeU16(bytes, machine + 10, 1);
    writeU16(bytes, machine + 12, 2);
    bytes[machine + 14] = 3;
    bytes[machine + 15] = 3;
    bytes[machine + 18 + 3] = 1;
    bytes[machine + 18 + 4 + 3] = 2;
    writeU16(bytes, machine + 26, 18);
    writeU16(bytes, machine + 34, 22);
    writeU16(bytes, machine + 36, 0x8000);
...
fn colrV1RadialGradientTable(allocator: std.mem.Allocator) ![]u8 {
    ...
    bytes[50] = 6; // PaintRadialGradient.
    writeU24(bytes, 51, 16);
    writeI16(bytes, 54, 350);
    writeI16(bytes, 56, 100);
    writeU16(bytes, 58, 0);
    writeI16(bytes, 60, 350);
    writeI16(bytes, 62, 100);
    writeU16(bytes, 64, 350);
...
fn mathTable(allocator: std.mem.Allocator) ![]u8 {
    const bytes = try allocator.alloc(u8, 356);
    @memset(bytes, 0);
    writeU16(bytes, 0, 1);
    writeU16(bytes, 2, 0);
    writeU16(bytes, 4, 10);
...
fn cff2Table(allocator: std.mem.Allocator) ![]u8 {
    const bytes = try allocator.alloc(u8, 71);
    @memset(bytes, 0);
    bytes[0] = 2;
    bytes[1] = 0;
    bytes[2] = 5;
    ...
    writeU32(bytes, 43, 1); // CFF2 FDArray INDEX count.
    bytes[47] = 1;
    bytes[48] = 1;
    bytes[49] = 4;
    ...
    writeU32(bytes, 62, 1); // CFF2 Local Subrs INDEX count.
    bytes[66] = 1;
    bytes[67] = 1;
    bytes[68] = 2;
    bytes[69] = 11;
...
fn featTable(allocator: std.mem.Allocator) ![]u8 {
    const bytes = try allocator.alloc(u8, 32);
    @memset(bytes, 0);
    writeU32(bytes, 0, 0x00010000);
    writeU16(bytes, 4, 1);
...
fn ltagTable(allocator: std.mem.Allocator) ![]u8 {
    const tag = "zh-Hant";
    const bytes = try allocator.alloc(u8, 16 + tag.len);
    @memset(bytes, 0);
    writeU32(bytes, 0, 1);
    writeU32(bytes, 4, 0);
    writeU32(bytes, 8, 1);
    writeU16(bytes, 12, 16);
    writeU16(bytes, 14, tag.len);
    @memcpy(bytes[16 .. 16 + tag.len], tag);
    return bytes;
}
...
fn vorgTable(allocator: std.mem.Allocator) ![]u8 {
    const bytes = try allocator.alloc(u8, 12);
    @memset(bytes, 0);
    writeU32(bytes, 0, 0x00010000);
    writeI16(bytes, 4, 880);
    writeU16(bytes, 6, 1);
    writeU16(bytes, 8, 1);
    writeI16(bytes, 10, 910);
    return bytes;
}
