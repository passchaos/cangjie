...
fn harfBuzzDesignVariations(allocator: std.mem.Allocator, coords: []const cangjie.font.metadata.variations.Coordinate) ![]hb.hb_variation_t {
...
fn scriptTagForText(text: []const u8) ?c_uint {
    ...
    while (it.nextCodepoint()) |codepoint| {
        ...
        return @intFromEnum(cangjie.text.opentype.script(script));
    }
    return null;
}
