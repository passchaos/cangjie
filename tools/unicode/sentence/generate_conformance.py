#!/usr/bin/env python3
"""Compile Unicode 17.0 SentenceBreakTest.txt into a compact fixture."""
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit(
            f"usage: {Path(sys.argv[0]).name} SentenceBreakTest.txt output.bin"
        )
    source = Path(sys.argv[1]).read_text(encoding="utf-8")
    if not source.startswith("# SentenceBreakTest-17.0.0.txt\n"):
        raise SystemExit("expected Unicode 17.0 SentenceBreakTest fixture")
    ...
    if len(cases) != 512:
        raise SystemExit(f"expected 512 Unicode 17 cases, found {len(cases)}")

    output = bytearray(struct.pack("<4sBBBBI", b"CJST", 1, 17, 0, 0, len(cases)))
